...
# mpi Auto Picking
`which relion_autopick_mpi` --i CtfFind/job003/micrographs_ctf.star --odir AutoPick/job006/ --pickname autopick --LoG  --LoG_diam_min 200 --LoG_diam_max 250 --shrink 0 --lowpass 20 --LoG_adjust_threshold 0  --pipeline_control AutoPick/job006/

# mpi PreProcess Particle Extraction
`which relion_preprocess` --i ManualPick/job005/micrographs_selected.star --coord_list AutoPick/job006/autopick.star --part_star Extract/job008/particles.star --part_dir Extract/job008/ --extract --extract_size 128 --float16  --norm --bg_radius 48 --white_dust -1 --black_dust -1 --invert_contrast   --pipeline_control Extract/job008/
